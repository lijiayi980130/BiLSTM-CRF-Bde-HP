...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_bertembed_file', nargs='+', default='',
                        help='path to embedding file')
    parser.add_argument('--test_bertembed_file', nargs='+', default='',
                        help='path to embedding file')
    ...
    if not args.rand_embedding:
        ...
        for i in range(file_num):
            word_bert0 = []
            with h5py.File(args.train_bertembed_file[i], 'r') as f:
                with open(args.train_bertembed_file[i].replace(".h5", ".txt"), 'r',encoding="utf-8") as f_in:
                    for id, input in enumerate(f_in):
                        entity_name = input.strip()
                        embedding = f[entity_name]['embedding'][:]

                        word_bert0.append(embedding)
            word_bert.append(word_bert0)
        ...
        for i in range(file_num):
            testword_bert0 = []
            with h5py.File(
                    args.test_bertembed_file[i],
                    'r') as f:
                with open(
                        args.test_bertembed_file[i].replace(".h5", ".txt"),
                        'r',encoding="utf-8") as f_in:
                    for id, input in enumerate(f_in):
                        entity_name = input.strip()
                        embedding = f[entity_name]['embedding'][:]

                        testword_bert0.append(embedding)
            testword_bert.append(testword_bert0)
    ...
    for i in range(file_num):
        ...
        dataset, forw_corp, back_corp = utils.construct_bucket_mean_vb_wc(
            train_features[i], train_labels[i], word_bert[i],l_map[i], char_map, f_map, args.caseless)
        test_dataset, forw_test, back_test = utils.construct_bucket_mean_vb_wc(
            test_features[i], test_labels[i], testword_bert[i],l_map[i], char_map, f_map, args.caseless)
        ...
        test_dataset_loader.append(
            [torch.utils.data.DataLoader(tup, 32, shuffle=False, drop_last=False) for tup in test_dataset])
    ...
    pn_layers = ["U", "W", "bilinnear", "decoder_lstm", "decoderout",\
                "decoderrepre_lstm", "query", "key", "value","weight_proj"]
    pn_params = []
    crf_params = []
    for name, param in ner_model.named_parameters():
        if name.split(".")[0] in pn_layers:
            pn_params.append(param)
        else:
    ...
    if args.update == 'sgd':
        optimizer = optim.SGD([{"params":iter(pn_params),"lr":args.lr*10},
                                {"params":iter(crf_params),"lr":args.lr}]
                              ,lr=args.lr, momentum=args.momentum)
    elif args.update == 'adam':
        optimizer = optim.Adam([{"params": pn_params, "lr": args.lr * 10}]
                              , lr=args.lr, momentum=args.momentum)
    ...
    for epoch_idx, args.start_epoch in enumerate(epoch_list):
        ...
        for i in tqdm(range(len(all_indices)), mininterval=2,
            ...
            f_f, f_p, b_f, b_p, w_f, tg_v, mask_v, len_v,s_t,mask1_v,e_t,mask2_v,bert_f = next(all_train_data_iter[file_no])

            f_f, f_p, b_f, b_p, w_f, tg_v, mask_v,s_t,mask1_v,e_t,mask2_v,bert_f = packer[file_no].repack_vb(f_f, f_p, b_f, b_p, w_f, tg_v, mask_v, len_v,s_t,mask1_v,e_t,mask2_v,bert_f)

            ner_model.zero_grad()

            scores,binclass_scores,all_attention = ner_model(f_f, f_p, b_f, b_p, w_f, bert_f,file_no)
            loss = crit_ner[file_no](scores, tg_v, mask_v,binclass_scores,all_attention,s_t,mask1_v,e_t,mask2_v)
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(ner_model.parameters(), args.clip_grad)
            optimizer.step()
        ...
        for file_no in range(file_num):
            ...
            if test_f1 > best_f1_test[file_no]:
                ...
                if args.output_annotation:  # NEW
                    print('annotating')
                    with open('output_3th_sep_coh_' + str(file_num) + '_' + str(file_no) + '.txt', 'w',encoding="utf-8") as fout:
