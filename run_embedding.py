        ...
        for inputs in tqdm(dataloader, desc='embedding'):
            ...
            if self.args.pooling == 'first':
                ...
                for each_metadata in metadata:
                    begin_token = []
                    id = 0
                    #print(each_metadata['text'])
                    while(id<len(each_metadata['tokens'])-1):
                        if(each_metadata['tokens'][id+1].startswith('##')):
                            temp = id
                            while(each_metadata['tokens'][id+1].startswith('##')):
                                id = id+1
                            begin_token.append((temp,id+1-temp))

                        id = id+1

                    #print(begin_token)

                    begin_tokens.append(begin_token)

                for embed, attention_mask,begin_token,each_metadata in zip(last_hidden_states, inputs['attention_mask'],begin_tokens,metadata):
                    token_embed = []
                    for each_begin in begin_token:
                        sum = embed[each_begin[0]]
                        for id in range(each_begin[0]+1,each_begin[0]+each_begin[1]):
                            sum = sum + embed[id]
                            attention_mask[id] = 0
                        embed[each_begin[0]] = sum/each_begin[1]

                    for id,mask in enumerate(attention_mask):
                        if(mask==1):
                            token_embed.append(embed[id])
                    if((len(token_embed)-2)!=len(each_metadata['text'].split())):
                           print(each_metadata['text'])
                           print(len(token_embed)-2)
                           print(len(each_metadata['text'].split()))
                    #assert len(token_embed)-2 == len(each_metadata['text'].split())
                
                    embeddings.append(torch.stack(token_embed[1:-1],0))
