...
def read_features_labels(lines): # NEW
    """
    ...
    for line in lines:
        ...
        if not (line.isspace() or if_doc_end):
            ...
            if if_doc_end and len(features) > 0:
                documents_f.append(features)
                documents_l.append(labels)
                features = list()
                labels = list()
    if len(tmp_fl) > 0:
        features.append(tmp_fl)
    ...
    if len(features) > 0:
        documents_f.append(features)
        documents_l.append(labels)
    return documents_f, documents_l
...
def encode_se(labels):
    start_tags = []
    end_tags = []
    masks = []
    for label in labels:
        start_tag = [0]*len(label)
        end_tag = [0]*len(label)
        mask = [1]*len(label)
        for idx,word_label in enumerate(label):
            if(word_label.startswith("B") or word_label.startswith("S")):
                start_tag[idx]=1
                if (word_label.startswith("B")):
                    tmp = idx
                    while not label[tmp].startswith("E"):
                        tmp = tmp + 1
                    end_tag[idx] = tmp
                elif (word_label.startswith("S")):
                    end_tag[idx] = idx
            elif (word_label.startswith("O")):
                    end_tag[idx] = -1
            else:
                mask[idx] = 0
        ...
        masks.append(mask)
    return start_tags,end_tags,masks



def construct_bucket_mean_vb_wc(word_features, input_label, word_bert,label_dict, char_dict, word_dict, caseless):
    """
    ...
    start_tags,end_tags,masks = encode_se(input_label)
    ...
    return construct_bucket_vb_wc(word_features, forw_features, word_bert,fea_len, labels, start_tags,end_tags,masks,thresholds, word_dict['<eof>'], char_dict['\n'], label_dict['<pad>'], len(label_dict))

def construct_bucket_vb_wc(word_features, forw_features, word_bert,fea_len, input_labels, start_tags,end_tags,masks,thresholds, pad_word_feature, pad_char_feature, pad_label, label_size):
    """
    ...
    # calc padding
    for f_f, f_l, w_f, b_f,i_l,s_t,e_t,mask in zip(forw_features, fea_len, word_features, word_bert,input_labels,start_tags,end_tags,masks):
        cur_len = len(f_l)
        idx = 0
        cur_len_1 = cur_len + 1
        while thresholds[idx] < cur_len_1:
        ...
        b_f0 = [[0.0 for col in range(768)] for row in range(thresholds[idx])]
        b_f0[:len(w_f)] = b_f.tolist()
        ...
        buckets[idx][8].append(s_t + [0]*(thresholds[idx]-cur_len))  #s_t
        buckets[idx][9].append([1]*cur_len+[0]*(thresholds[idx]-cur_len))  #mask1
        buckets[idx][10].append(e_t+[0]*(thresholds[idx]-cur_len))
        buckets[idx][11].append(mask + [0]*(thresholds[idx]-cur_len))
        buckets[idx][12].append(b_f0)
    bucket_dataset = [CRFDataset_WC(torch.LongTensor(bucket[0]), torch.LongTensor(bucket[1]),
                                   ...
                                   torch.LongTensor(bucket[10]),torch.ByteTensor(bucket[11]),
                                   torch.FloatTensor(bucket[12])) for bucket in buckets]
