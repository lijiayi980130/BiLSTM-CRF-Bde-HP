...
class CRFDataset_WC(Dataset):
    ...
    """
    def __init__(self, forw_tensor, forw_index, back_tensor, back_index, word_tensor, label_tensor, mask_tensor, len_tensor,starttag_tensor,
                 mask1_tensor,endtag_tensor,mask2_tensor,bertf_tensor):
        ...
        assert forw_tensor.size(0) == starttag_tensor.size(0)
        assert forw_tensor.size(0) == endtag_tensor.size(0)
        assert forw_tensor.size(0) == mask1_tensor.size(0)
        assert forw_tensor.size(0) == mask2_tensor.size(0)
        assert forw_tensor.size(0) == bertf_tensor.size(0)
        ...
        self.starttag_tensor = starttag_tensor
        self.endtag_tensor = endtag_tensor
        self.mask1_tensor = mask1_tensor
        self.mask2_tensor = mask2_tensor
        self.bertf_tensor = bertf_tensor
    ...
    def __getitem__(self, index):
        return self.forw_tensor[index], self.forw_index[index], self.back_tensor[index], self.back_index[index], \
               ...
               self.starttag_tensor[index], self.mask1_tensor[index], self.endtag_tensor[index], self.mask2_tensor[index],\
               self.bertf_tensor[index]
