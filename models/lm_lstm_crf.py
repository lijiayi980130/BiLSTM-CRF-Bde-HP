...
class LM_LSTM_CRF(nn.Module):
    ...
    args: 
        tagset_size: size of unified label set
    ...
    def __init__(self, tagset_size, char_size, char_dim, char_hidden_dim, char_rnn_layers, embedding_dim, word_hidden_dim, word_rnn_layers, vocab_size, dropout_ratio, file_num, large_CRF=True, if_highway=False, in_doc_words=2, highway_layers=1):
        ...
        for i in range(file_num):    # a separate CRF for each dataset
            if large_CRF:
...
class LM_LSTM_CRF_sep(LM_LSTM_CRF):   # extends LM_LSTM_CRF with separate CRFs and label sets
    def __init__(self, tagset_size, char_size, char_dim, char_hidden_dim, char_rnn_layers, embedding_dim, word_hidden_dim, word_rnn_layers, vocab_size, dropout_ratio, file_num, large_CRF=True, if_highway=False, in_doc_words=2, highway_layers=1):
        ...
        self.word_lstm = nn.LSTM(embedding_dim, word_hidden_dim // 2, num_layers=word_rnn_layers, bidirectional=True, dropout=dropout_ratio)
        ...
        print("large_CRF: ", large_CRF)
        for i in range(file_num):  # a separate CRF for each dataset with an individual label set
            if large_CRF:
                self.crflist.append(crf.CRF_L(word_hidden_dim, tagset_size[i]))
            else:
                self.crflist.append(crf.CRF_S(word_hidden_dim, tagset_size[i]))
        ...
        self.decoderrepre_lstm = nn.LSTM(embedding_dim, word_hidden_dim // 2, num_layers=word_rnn_layers,
                                         bidirectional=True,dropout=dropout_ratio)
        self.decoderout = nn.Linear(word_hidden_dim,2)

        self.query = nn.Linear(word_hidden_dim, word_hidden_dim,bias=False)
        self.key = nn.Linear(word_hidden_dim, word_hidden_dim,bias=False)
        self.value = nn.Linear(word_hidden_dim, word_hidden_dim,bias=False)

        self.decoder_lstm = nn.LSTM(embedding_dim+word_hidden_dim,word_hidden_dim,num_layers=word_rnn_layers,
                                    bidirectional=False,dropout=dropout_ratio)
        self.W = nn.Linear(word_hidden_dim,word_hidden_dim,bias=False)
        self.U = nn.Linear(word_hidden_dim,word_hidden_dim,bias=False)
        self.weight_proj = nn.Linear(word_hidden_dim, 1,bias=False)
        self.bilinnear = nn.Bilinear(word_hidden_dim,word_hidden_dim,1)

    def forward(self, forw_sentence, forw_position, back_sentence, back_position, word_seq, bertembed_feature,file_no, hidden=None):
        '''
        args:
            forw_sentence (char_seq_len, batch_size) : char-level representation of sentence
            forw_position (word_seq_len, batch_size) : position of blank space in char-level representation of sentence
        ...
        # bert_embedding
        #d_bertembed_feature = self.dropout(bertembed_feature)

        # combine
        #word_input = torch.cat((d_word_emb, dim=2)
        '''
        word_emb = word_emb.permute(1, 0, 2).detach().numpy().tolist()

        for sen in word_emb:
            sen.append([0] * len(sen[0]))
            sen.insert(0, [0] * len(sen[0]))

        word_emb = torch.Tensor(word_emb)

        _word_emb = word_emb[:, :-2, :]
        _word_emb_ = word_emb[:, 1:-1, :]
        word_emb_ = word_emb[:, 2:, :]

        word_emb = torch.cat((_word_emb,_word_emb_,word_emb_),dim=2)
        ...
        for sen in word_emb:
            for idx in range(1,len(sen)-1):
                for j in range(len(sen[0])):
                    sen[idx][j] = sen[idx-1][j]+sen[idx][j]+sen[idx+1][j]

            sen.pop()
            sen.pop(0)

        word_emb = torch.Tensor(word_emb).permute(1,0,2)
        '''
        ...
        # combine
        #word_input = torch.cat((d_word_emb, bertembed_feature), dim=2)
        word_input = d_word_emb
        ...
        #decoder
        decoder_emb,_ = self.decoderrepre_lstm(word_input)
        decoder_binclass = self.decoderout(decoder_emb)
        # decoder_out = nn.functional.softmax(decoder_out,dim=2)

        transdecoder_emb = decoder_emb.permute(1,0,2)

        #decoder self_attention
        mixed_query_layer = self.query(transdecoder_emb)
        mixed_key_layer = self.key(transdecoder_emb)
        mixed_value_layer = self.value(transdecoder_emb)

        atten = torch.matmul(mixed_query_layer, mixed_key_layer.transpose(2,1))
        atten_score = nn.functional.softmax(atten, dim=2)

        globaldecoder_emb = torch.matmul(atten_score,mixed_value_layer)


        inactive = torch.zeros(1,lstm_out.size(1),lstm_out.size(2))
        decoder_hidden = torch.cat((d_lstm_out,inactive),dim=0)


        decoder_input = torch.cat((word_input,globaldecoder_emb.transpose(1,0)),dim=2)

        hidden = (d_lstm_out[-1].unsqueeze(0),d_lstm_out[-1].unsqueeze(0))

        decoder_output,_= self.decoder_lstm(decoder_input,hidden)

        all_boundary = []

        #decoder encoder_attention
        for out in decoder_output:
            out = out.expand(decoder_hidden.size(0),out.size(0),out.size(1))
            temp = self.W(decoder_hidden)+self.U(out)
            allatten = self.weight_proj(temp)+self.bilinnear(out.contiguous(), decoder_hidden.contiguous())
            allatten = allatten.squeeze(2)
            #endboundary_socre = nn.functional.softmax(allatten,dim=0).squeeze(2)
            all_boundary.append(allatten)

        #print(len(all_boundary))
        #print(all_boundary[0].shape)

        all_boundary = torch.stack(all_boundary)



        #print(all_boundary.shape)

        d_lstm_out_ = torch.cat((d_lstm_out,decoder_emb),dim=2)

        # convert to crf
        crf_out = self.crflist[file_no](d_lstm_out)    # corresponding crf
        crf_out = crf_out.view(self.word_seq_length, self.batch_size, self.tagset_size[file_no], self.tagset_size[file_no])

        return crf_out,decoder_binclass,all_boundary
