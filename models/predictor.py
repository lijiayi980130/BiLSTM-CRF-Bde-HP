...
class predict:
    ...
    def decode_l(self, feature, g_label, label):
        """
        ...
        """
        return '\n'.join(map(lambda t: t[0] + '\t' + t[1] + '\t' + self.r_l_map[t[2]], zip(feature, g_label, label.cpu().numpy())))

    def decode_s(self, feature, g_label, label):
        """
    ...
    def output_batch(self, ner_model, documents, gold_labels, fout, file_no):
        """
        ...
        for d_ind in tqdm(range(0, d_len), mininterval=1,
            ...
            g_labels = gold_labels[d_ind]
            f_len = len(features)
            # print("f_len:", f_len, " g_len:", len(g_labels))
            for ind in range(0, f_len, self.batch_size):
                ...
                for ind2 in range(ind, eind):
                    f = features[ind2]
                    g_l = g_labels[ind2]
                    p_l = labels[ind2 - ind][0: len(f)]
                    fout.write(self.decode_str(features[ind2], g_l,p_l) + '\n')
...
class predict_wc(predict):
    ...
    def apply_model(self, ner_model, features,file_no):
        ...
        for feature in features:
            b_f0 = [[0.0 for col in range(768)] for row in range(word_len)]
            #b_f0[:len(embed)] = embed
            b_f1.append(b_f0)

        # print(len(b_f1))
        # print(len(b_f1[0]))

        bert_f = torch.FloatTensor(b_f1)
        ...
        if self.if_cuda:
            ...
            mask_v = masks.transpose(0, 1).cuda()
            bert_f = autograd.Variable(bert_f.transpose(0, 1)).cuda()
        else:
            ...
            bert_f = autograd.Variable(bert_f.transpose(0, 1))
