...
class CRF_L(nn.Module):    # large CRF
...
class CRFRepack_WC:
    ...
    def __init__(self, tagset_size, if_cuda):
        ...
        self.tagset_size = tagset_size
        self.if_cuda = if_cuda
    # Variable后，char和word embedding都是要用导数更新的
    def repack_vb(self, f_f, f_p, b_f, b_p, w_f, target, mask, len_b,s_t,mask1,e_t,mask2,bert_f):
        """packer for viterbi loss
        ...
        if self.if_cuda:
            ...
            s_t = autograd.Variable(s_t[:, 0:mlen[1]].transpose(0, 1)).cuda()
            mask1 = autograd.Variable(mask1[:, 0:mlen[1]].transpose(0, 1)).cuda()
            e_t = autograd.Variable(e_t[:, 0:mlen[1]].transpose(0, 1)).cuda()
            mask2 = autograd.Variable(mask2[:, 0:mlen[1]].transpose(0, 1)).cuda()
            bert_f = autograd.Variable(bert_f[:, 0:mlen[1]].transpose(0, 1)).cuda()
        else:
            ...
            s_t = autograd.Variable(s_t[:, 0:mlen[1]].transpose(0, 1))
            mask1 = autograd.Variable(mask1[:, 0:mlen[1]].transpose(0, 1))
            e_t = autograd.Variable(e_t[:, 0:mlen[1]].transpose(0, 1))
            mask2 = autograd.Variable(mask2[:, 0:mlen[1]].transpose(0, 1))
            bert_f = autograd.Variable(bert_f[:, 0:mlen[1]].transpose(0, 1))

        return f_f, f_p, b_f, b_p, w_f, tg_v, mask_v,s_t,mask1,e_t,mask2,bert_f
...
class CRFLoss_vb(nn.Module):
    ...
    def forward(self, scores, target, mask,decoder_binclass,all_boundary,start_target,mask1,end_target,mask2):
        """
        args:
        ...
        # print(target)
        # print(mask)
        end_target = end_target.permute(1, 0).numpy()
        end_target[end_target==-1] = end_target.shape[1]

        end_target = torch.from_numpy(end_target)
        ...
        # average = mask.sum()
        start_target = start_target.unsqueeze(-1)

        logsoftmax_out = self.logsoftmax(decoder_binclass)
        tgloss1 = torch.gather(logsoftmax_out,dim=2,index=start_target)

        tgloss1 = tgloss1.squeeze(-1)

        tgloss1 = -(tgloss1.masked_select(mask1.bool()).sum() / bat_size)

        all_boundary = all_boundary.permute(2,0,1)
        all1_boundary = self.softmax(all_boundary)

        end_target = end_target.unsqueeze(2)

        logsoftmax_out1 = self.logsoftmax(all_boundary)
        tgloss2 = torch.gather(logsoftmax_out1, dim=2,index=end_target)

        tgloss2 = tgloss2.squeeze(-1).permute(1,0)

        tgloss2 = -(tgloss2.masked_select(mask2.bool()).sum() / bat_size)
...
class CRFDecode_vb():
        ...
        def decode(self, scores, mask):
            ...
            decode_idx[-1] = pointer  # [50]
            for idx in range(len(back_points) - 2, -1, -1):
                # 输入是一维的，gather的第三个参数也是一维的，指明了需要输出元素在输入中的位置，个数不限。
                pointer = torch.gather(back_points[idx], 1, pointer.contiguous().view(bat_size, 1))  # [50, 1]
                decode_idx[idx] = pointer.contiguous().view(
                    bat_size)  # decode_idx的shape是[seq_len, bat_size]。所以pointer只能是一维的
            return decode_idx

        def decodePN(self, binclass_scores, all_attention, mask1, l_map):

            """
            Find the prediction starttag and endtag

            args:
            binclass_score (seq_len,batch_size,2)
            all_attention (seq_len,seq_len+1 ...inactive,batch_size)
            mask1  (seq_len,batch_size)
            """

            binclass_scores = binclass_scores.permute(1, 0, 2)
            startP = self.softmax(binclass_scores)
            _, startIdx = torch.max(startP, 2)

            all_attention = all_attention.permute(2, 0, 1)
            allP = self.softmax(all_attention)
            _, endIdx = torch.max(allP, 2)

            mask1 = mask1.permute(1, 0)

            startIdx = startIdx.numpy().tolist()
            endIdx = endIdx.numpy().tolist()
            mask1 = mask1.numpy().tolist()

            decodelabels = []

            for e_i, mask in zip(endIdx, mask1):
                decodelabel = []
                tag = -1
                for e, m in zip(enumerate(e_i), mask):
                    if (m == 1):
                        if (e[0] <= tag):
                            continue
                        elif (e[1] == len(e_i)):
                            decodelabel.append("O")
                        elif (e[1] == e[0]):
                            decodelabel.append("S")
                        elif (e[1] > e[0]):
                            tag = e[1]
                            decodelabel.append("B")
                            temp = e[0] + 1
                            while (temp < e[1]):
                                decodelabel.append("I")
                                temp = temp + 1
                            decodelabel.append("E")
                        else:
                            decodelabel.append("O")
                    else:
                        decodelabel.append("<")  # pad填充
                decodelabels.append(decodelabel)

            l_lists = l_map.items()
            temp_map = {}
            for l_list in l_lists:
                temp_map[l_list[0][0]] = l_list[1]

            decodeIdx = []
            for sen in decodelabels:
                sen_idx = []
                for label in sen:
                    sen_idx.append(temp_map[label])
                decodeIdx.append(sen_idx)

            decode_Idx = torch.Tensor(decodeIdx).permute(1, 0)  # decodeIdx的shape是
            # (seq_len,bat_size)
            return decode_Idx
